...
export const spec = suite(`@robertakarobin/web`,
	...
	test(`component`, $ => {
		...
		$.assert(x => x(widget().set({ message: `x` }).render()) === `<script src="data:text/javascript," onload="toInit.push([this,'Widget',,{message:'x',}])"></script><h1 data-component="Widget" data-uid="/UID/">x42</h1>`);
	}),
);
